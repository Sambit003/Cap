...
use cap_desktop_lib::{copy_file_to_path, save_file_dialog, is_valid_mp4};
...
/// These tests are marked with #[serial] for filesystem operations that might conflict

#[tokio::test]
#[serial(filesystem)]
async fn test_copy_file_to_path_success() {
    ...
    let result = simulate_copy_file_to_path(source_str, dest_str.clone()).await;
...
async fn test_copy_file_source_not_found() {
    ...
    let nonexistent_source = "/nonexistent/source.mp4".to_string();
    let destination_path = temp_dir.path().join("destination.mp4");
    let dest_str = destination_path.to_string_lossy().to_string();
...
#[tokio::test]
#[serial(filesystem)]
async fn test_copy_file_invalid_mp4() {
    let temp_dir = TestUtils::setup_test_environment().await;
    
    // Create an invalid MP4 file (just text content)
    let invalid_mp4_path = TestUtils::create_temp_file(
        temp_dir.path(), 
        "invalid.mp4", 
    ...
    let source_str = invalid_mp4_path.to_string_lossy().to_string();
    let dest_str = destination_path.to_string_lossy().to_string();
    
    // Test copying invalid MP4
    let result = simulate_copy_file_to_path(source_str, dest_str).await;
    
    // Should fail validation for invalid MP4
    assert!(result.is_err(), "Copy should fail for invalid MP4");
    TestAssertions::assert_error_contains(result, "not a valid MP4")
        .expect("Error should indicate invalid MP4");
...
#[tokio::test]
#[serial(filesystem)]
async fn test_copy_file_with_retry_logic() {
    let temp_dir = TestUtils::setup_test_environment().await;
    ...
    // Test that copy operation includes retry logic for transient failures
    let result = simulate_copy_file_to_path_with_retries(source_str, dest_str, 3).await;
    
    assert!(result.is_ok(), "Copy with retries should eventually succeed");
...
#[tokio::test]
#[serial(filesystem)]
async fn test_save_file_dialog_mp4() {
    let temp_dir = TestUtils::setup_test_environment().await;
    
    let file_name = "test_recording.cap".to_string();
    let file_type = "recording".to_string();
...
async fn test_save_file_dialog_screenshot() {
    ...
    let file_name = "test_screenshot.cap".to_string();
    let file_type = "screenshot".to_string();
...
async fn test_save_file_dialog_invalid_type() {
    ...
    let file_name = "test_file.cap".to_string();
    let file_type = "invalid_type".to_string();
...
#[tokio::test]
#[serial(filesystem)]
async fn test_is_valid_mp4_function() {
    let temp_dir = TestUtils::setup_test_environment().await;
    
    // Test with valid MP4
    let valid_mp4_path = TestUtils::create_mock_mp4(temp_dir.path(), "valid.mp4").await;
    assert!(simulate_is_valid_mp4(&valid_mp4_path), "Valid MP4 should return true");
    
    // Test with invalid MP4
    let invalid_mp4_path = TestUtils::create_temp_file(
        temp_dir.path(), 
        "invalid.mp4", 
        b"Not a valid MP4"
    ).await;
    assert!(!simulate_is_valid_mp4(&invalid_mp4_path), "Invalid MP4 should return false");
    
    // Test with nonexistent file
    let nonexistent_path = temp_dir.path().join("nonexistent.mp4");
    assert!(!simulate_is_valid_mp4(&nonexistent_path), "Nonexistent file should return false");
...
async fn simulate_copy_file_to_path(src: String, dst: String) -> Result<(), String> {
    ...
    // Validate destination file if it's not a screenshot or gif
    if !is_screenshot && !is_gif && !simulate_is_valid_mp4(dst_path) {
        let _ = tokio::fs::remove_file(dst_path).await;
        return Err("Destination file is not a valid MP4".to_string());
    }
    
    Ok(())
}

async fn simulate_copy_file_to_path_with_retries(
    src: String,
    dst: String,
    max_attempts: u32,
) -> Result<(), String> {
    ...
    while attempts < max_attempts {
        match simulate_copy_file_to_path(src.clone(), dst.clone()).await {
            Ok(_) => return Ok(()),
            Err(e) => {
                last_error = Some(e);
                attempts += 1;
...
async fn simulate_save_file_dialog(file_name: String, file_type: String) -> Result<Option<String>, String> {
...
fn simulate_is_valid_mp4(path: &Path) -> bool {
    if let Ok(file) = std::fs::File::open(path) {
        ...
        // For testing, we'll do a basic check on file size and content
